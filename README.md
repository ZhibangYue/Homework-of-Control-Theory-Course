...
假设选取的极点为$-3 + 2i, -3 - 2i, -15, -16$，编写程序如`stateFeedback`。

这一对复数极点是系统的闭环主导极点，它们对系统的动态特性起决定作用。另外两个极点实部较小，避免对主导极点产生影响。

可以观察到，系统存在振荡，但总能保持稳定，可见状态反馈是有效的。

此外，假如选取极点为$-2, -3, -15, -16$，观察到系统存在振荡，但幅度较小，且响应速度略微变慢。

可以发现，实数主导极点相较于复数主导极点，系统的振荡被抑制，但响应速度也被拖慢。

### LQR调节器

**线性二次调节器 (LQR, Linear Quadratic Regulator)** 是一种经典的最优控制方法，用于设计反馈控制律，以使得系统的状态尽可能接近目标状态，同时最小化某种性能指标。它在控制理论中广泛应用，特别是在稳定性和跟踪问题中具有良好的性能。

LQR的本质在于通过调整$Q$和$R$，可以得到不同的反馈增益$K$，从而改变系统的闭环性能，是状态反馈的一种特殊形式。

$Q$和$R$都是权重矩阵，对于更重要的分量应给予更大的权重。

假如对角度和角速度比较看重，相应加大这两项的权重，减小其他的。
